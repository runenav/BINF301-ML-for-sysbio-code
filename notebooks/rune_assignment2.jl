...
md"
1. **Loading data and intial processing**
"

# ╔═╡ 77dee35e-f3be-48b1-9969-ea5fb77961d6
begin
	cd("C:/Users/runen/OneDrive/Dokumenter/UiB/Postmaster/BINF301/part2/BINF301-ML-for-sysbio-code/notebooks")
	pwd()
end

# ╔═╡ 652b0ed3-075a-478a-9035-f73faa69a3ff
cd("..")

# ╔═╡ 528ae44b-ff1b-4ea9-8700-afa0f52af9f9
begin
	# Loading data
	data_clinical = open("data/processed/TCGA_BRCA/TCGA-BRCA-exp-348-clin.csv");
	data_expressn = open("data/processed/TCGA_BRCA/TCGA-BRCA-exp-348-expr.csv");

	# Creating dataframes from loaded data
	df_clin = CSV.read(data_clinical, DataFrame);
	df_expr = CSV.read(data_expressn, DataFrame);
...
# ╔═╡ 859d7916-fe84-4022-9b79-b465ec0cb0d5
begin
	# Finding size of expression data
	# Samples are stored as rows; Genes stored as columns
	n_samples, n_genes = size(df_expr);
	println("Number of samples: ", n_samples)
	println("Number of genes:   ", n_genes)
end

# ╔═╡ 5c56bcda-6faf-4852-a7e7-2fd1a2e9699c
md"
Identifying and labelling samples with negative status in all three genes: ER, PR and HER2. These samples will be referred to as negative; all others will be referred to positive.

The number of non-triple negatives is much higher than triple negatives. We must create a dataset of equal numbers of both.
"

# ╔═╡ dd675689-3832-4bd4-8d37-3294e34335f3
begin
	# Checking which samples are triple negative
	triple_neg = df_clin.:"ER Status" .== "Negative" .&& df_clin.:"PR Status" .== "Negative" .&& df_clin.:"HER2 Final Status" .== "Negative";

	# Finding the indices of which samples are triple negative or not
	index_neg = findall(triple_neg);
	index_pos = findall(.!triple_neg);
	...
	# Sanity check
	println("Triple negatives:     ", n_neg)
	println("Non-triple negatives: ", n_pos)
	println("Total:                ", n_neg + n_pos)
	println("Sanity check:         ", nrow(df_clin) == nrow(df_expr) == n_neg + n_pos);

	println("\nSubsampling positive samples to same size as negatives...");
	
	# Creating a random subsample of positives to be same size as negatives
	index_pos = shuffle(index_pos);# First, randomly shuffle the indices
	index_pos = index_pos[1:n_neg];# Second, subsample the balanced number of indices
	index_pos = sort(index_pos);   # Lastly, re-order the subsampled indices

	# Update number of non-triple negatives
	n_pos = length(index_pos);

	println("Same size:   ", n_neg == n_pos);
end

# ╔═╡ cf497de4-0c14-4765-95fd-99c5fd8cd63b
md"
Now, using the same-sized indices for positive and negatives, creating a dataframe for genes. A complementary vector holds the triple-neg labels for each row.
"
...
git-tree-sha1 = "5efdba15600ac456a04616942a35e7827ef1a410"
